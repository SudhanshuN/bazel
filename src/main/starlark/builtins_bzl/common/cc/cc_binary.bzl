...
def cc_binary_impl(ctx, additional_linkopts):
    ...
    outfile = {}
    for suffix in ctx.attr.additional_linker_output_suffixes:
        outfile[suffix] = ctx.actions.declare_file(_strip_extension(binary) + "." + suffix, sibling = binary)
        additional_linker_outputs.append(outfile[suffix])
    ...
    for suffix in ctx.attr.additional_linker_output_suffixes:
        output_groups[suffix] = depset([outfile[suffix]])
